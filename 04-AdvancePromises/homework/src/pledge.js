...
function $Promise(executor){
  ...
  this._handlerGroups = []; //success handler y  error handler consecutivamente. Porque javascript todavia no sabe el valor del sucess ni del error entonces por eso los guarda aca
...
$Promise.prototype.then = function(successCb, errorCb) {
  ...
  this._handlerGroups.push({successCb, errorCb, downstreamPromise}) // propiedades
...
$Promise.prototype._callHandlers =  function() {
  while(this._handlerGroups.length > 0){
    let current = this._handlerGroups.shift(); //[{sh1, eh1}]
    if(this._state === 'fulfilled') {
      if(!current.successCb) {
        ...
        try{
        const result = current.successCb(this._value) // si esto tiene un error va al catch
